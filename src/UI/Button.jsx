...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    ...
    secondary:
      'inline-block text-sm rounded-full border-2 border-stone-300 font-semibold uppercase tracking-wide text-stone-400 transition-colors duration-300 hover:bg-stone-300 hover:text-stone-800 focus:bg-stone-100 focus:text-stone-800 focus:outline-none focus:ring focus:ring-stone-200 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-2.5 sm:px-6 sm:py-3.5 ',
    round: base + 'px-2.5 py-1 md:px-3.5 ms:py-2 text-sm',
  };
  if (to) {
    return (
